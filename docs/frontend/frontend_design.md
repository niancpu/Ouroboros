...
1. 沙盘初始化控制台：`Configuration Matrix`
2. 实时推演画板：`Live Telemetry Canvas`
3. 智能体解剖室：`Entity Inspector`
4. 时间轴编辑器 / 剧本管理：`Chronos Script Editor`
5. 最终判决与漏洞白皮书：`Vulnerability Dossier`
...
| 颜色 | 值 | 用途 |
| :--- | :--- | :--- |
| 纯白 | `#ffffff` | 页面主体、图表背景、报告纸面 |
| 纯黑 | `#000000` | 文本、边框、表格线、下跌实体 |
| 克莱因蓝 | `#002fa7` | 运行态、选中态、关键曲线、预警标记 |
...
4. 白底上使用纯黑 1px/2px 线；蓝底上使用纯白 1px 线。
...
```text
┌──────────────────────────────────────────────────────────────┐
│ [OUROBOROS // NEW SIMULATION]       MARKET 000001.SH  3021.44 │
├──────────────────────────┬───────────────────────────────────┤
│ 标的与环境 40%            │ 智能体配平 60%                     │
│                          │ ┌────────┬────────┬────────┐      │
│ Ticker                   │ │ 公募   │ 游资   │ 散户   │      │
│ Chronos Anchor           │ │ Radar  │ Radar  │ Radar  │      │
│ Scenario                 │ │ Params │ Params │ Params │      │
├──────────────────────────┴───────────────────────────────────┤
│ [INITIATE SEQUENCE]                                           │
└──────────────────────────────────────────────────────────────┘
```
...
- 高度约 15vh。
- 左侧 Georgia 大标题：`[OUROBOROS // NEW SIMULATION]`
- 右侧纯黑块，白色等宽字体展示真实市场指数或系统时间。
...
```text
┌──────────────────────────────────────────────────────────────┐
│ OUROBOROS LIVE     SESSION sim_001     TICK 14     RUNNING    │
├──────────────┬───────────────────────────────────┬───────────┤
│ Chronos Feed │ Consensus Infection Canvas         │ LOB       │
│ 20% 蓝底      │ 上 60%: 拓扑图谱                   │ 20%       │
│              │ 下 40%: 价格 / 共识曲线             │           │
└──────────────┴───────────────────────────────────┴───────────┘
```
...
- 圆形只允许：
  ...
  - 选中态克莱因蓝实心 + 白字。
- 禁止阴影、渐变、发光、呼吸动画。
- 标签用引线牵出，等宽字体标注，例如 `[AGT-04_RTL] ACTIVE`。
...
英文名：`Entity Inspector`

### 定位

审计单个 Agent 的状态、资产、信念轨迹和脱敏推理摘要。体现系统可解释性。

### 布局

```text
┌──────────────────────────────────────────────────────────────┐
│ ENTITY INSPECTOR     [AGT-04_RTL]                            │
├──────────────────┬───────────────────────────────────────────┤
│ Roster 25%        │ 四象限工作区 75%                          │
│                  │ ┌──────────────┬────────────────────────┐ │
│ [AGT-01_WHALE]   │ │ 资产表        │ 脱敏审计流              │ │
│ [AGT-04_RTL]     │ ├──────────────┤                        │ │
│ ...              │ │ 信念轨迹      │                        │ │
│                  │ └──────────────┴────────────────────────┘ │
└──────────────────┴───────────────────────────────────────────┘
...
- 关键逻辑词汇可用克莱因蓝高亮，例如 `Margin Call`、`Panic`、`Liquidity Vacuum`。

### COT 边界

普通 Entity Inspector 不展示原始 `<thought>` 或 COT。它只能展示：
...
Chronos 页面属于第一版五页范围，但第一版以只读展示和剧本草案预览为主。它涉及未来 Tick、事实释放和剧本插桩，写入或正式编辑能力必须后续另行设计权限和审计。

### 布局

```text
┌──────────────────────────────────────────────────────────────┐
│ CHRONOS SCRIPT EDITOR                                        │
├──────────────────────────────────────────────────────────────┤
│ Timeline: ───○────○────●────△────○────○──────────────────── │
├──────────────────────────────┬───────────────────────────────┤
│ OFFICIAL TRACK                │ RUMOR TRACK                   │
│ 报纸式官方事件                 │ 电报式情绪消息                 │
└──────────────────────────────┴───────────────────────────────┘
```
...
```text
┌──────────────────────────────────────────────────────────────┐
│ COLLAPSE PROBABILITY: 87.4%             2026-05-17 / DEMO    │
├──────────────────┬──────────────────────┬───────────────────┤
│ 概述与定性        │ 图表证据              │ 虚拟龙虎榜 / 附录  │
│                  │ Sankey / Curves       │ Tables            │
└──────────────────┴──────────────────────┴───────────────────┘
```
...
- 可以打印。
- 用经典报纸三列版式。
- 主要内容用黑白，克莱因蓝只标记关键证据路径。

页眉：

- 左上巨大 Georgia 标题。
- 示例：`COLLAPSE PROBABILITY: 87.4%`
- 右上等宽字体：
  - `session_id`
  - `symbol`
  - `start_tick_id`
...
- `Configuration Matrix` 第一版是否允许创建真实会话，还是只生成本地草稿后交由 API 确认。
- `Chronos Script Editor` 第一版是否允许保存本地剧本草案；正式写入 Chronos SSOT 必须后置。
- `Entity Inspector` 是否需要角色权限区分，例如普通演示视图和内部审计视图。
- `Vulnerability Dossier` 的 collapse probability 由后端计算还是前端只展示后端字段。
