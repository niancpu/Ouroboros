...
Meta-Orchestrator 是 Ouroboros 的控制面。它负责全局 Agent 管理、时间推进、生命周期调度和返回体路由切割，但不参与具体金融逻辑计算，也不拥有业务数据的唯一真理源。

它可以像游戏引擎一样管理所有 Agent，但不得把 Agent A 的私有信息塞进 Agent B 的上下文。
...
Meta-Orchestrator 在推演开始时读取配置，实例化 Agent 进程或协程，并调用 Layer 3 初始化资金池和初始持仓。

每个 Tick 结束后，它读取 Layer 3 的账户快照或风险结果，处理强平、停用、恢复等生命周期动作。
...
- `agent_id`、`tick_id`、trace id -> 进入控制面日志。
...
- 超时 Agent 的默认动作：使用 `HOLD` 最安全，但会弱化恐慌行情；强制撤单更接近风控系统；保留上一 Tick 意图风险最高。
